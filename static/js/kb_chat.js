...
function addAssistantAnswer(answer, sources, data = {}) {
    ...
    if (assistantMode === "database" || data.columns || data.rows || data.payload) {
...
function openShareSessionModal(sessionId) {
    closeSessionMenu();
    ...
    openModal({
        title: "Share chat",
        message: "Copy this URL to share the chat.",
        input: true,
        readOnly: true,
        defaultValue: url,
        ...
        onConfirm: () => {
            ...
            showToast("Chat URL copied.", "success");
        }
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.addEventListener("click", (event) => {
        ...
        if (menu && !menu.contains(event.target) && !event.target.closest(".session-menu-btn")) {
            closeSessionMenu();
        }
    });
